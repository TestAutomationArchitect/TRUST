...
const COVERAGE_FORECAST = [
  ...
  { key: "agent.toolProbe.prompt + .successIndicators", when: (c) => section(c, "agent").value?.runtimeEndpoint, controls: ["AGENT-TOOL-ABUSE"], what: "a request needing a tool call beyond the caller's entitlement, and what a successful one looks like — add agent.toolProbe.endpoint too if the tools live behind an orchestrator rather than the agent endpoint the other probes use" },
